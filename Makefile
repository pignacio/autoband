...
lint:
	...
	-pep8 autoband tests
	-find autoband -name "*.py" -exec pylint {} +
...
test-cover: test-deps clean-pyc
	python setup.py nosetests --with-coverage --cover-package=autoband
...
coverage: test-deps clean-pyc
	coverage run --source autoband setup.py nosetests
	make coverage-show
...
docs: docs-deps
	rm -f docs/autoband.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ autoband
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
yapf:
	find autoband -name "*.py" -exec echo {} \; -exec yapf --diff {} \;
